...
class query extends database{
        ...
        public function getInvData(){
          $sql="SELECT * FROM inventory WHERE return_exp='0' ORDER BY id DESC";
          $result=$this->connect()->query($sql);
          if ($result->num_rows>0) {
            $arr=array();
            while($row=$result->fetch_assoc()){
        ...
        public function getExpListReturn(){
          $sql="SELECT * FROM inventory WHERE return_exp='1' ORDER BY return_date DESC";
          $result=$this->connect()->query($sql);
          if ($result->num_rows>0) {
            $arr=array();
            while($row=$result->fetch_assoc()){
        ...
        public function getStockList(){
          $sql="SELECT * FROM inventory WHERE return_exp='0' ";
          $result=$this->connect()->query($sql);
        ...
        public function getExpiredList(){
          $today=date("Y-m-d");
          $sql="SELECT * FROM inventory WHERE expire_date<'$today' AND return_exp='0' ORDER BY id DESC";
          $result=$this->connect()->query($sql);
          if ($result->num_rows>0) {
            $arr=array();
            while($row=$result->fetch_assoc()){
    ...
    // close getPurchData function
      ...
      class SalesItem extends database{
        ...
        FROM purchase AS ph 
        JOIN inventory AS inv ON inv.id=ph.inv_id 
        WHERE inv.item_name='$item' AND ph.qyt>0 AND inv.return_exp='0'
        ORDER BY id ASC";
          $result=$this->connect()->query($sql);
          if ($result->num_rows>0) {
            $arr=array();
        ...
        public function getInvItemNameBatchNumLis(){
          $sql="SELECT DISTINCT batch_no FROM inventory WHERE return_exp='0'";
          $result=$this->connect()->query($sql);
          if ($result->num_rows>0) {
            $arr=array();
            while($row=$result->fetch_assoc()){
      ...
      class CardProcessInvUpd extends database{
        public function getSalesData($idstr){
          ...
          $arr=array();
          foreach($arr_id as $key => $value){
            $sql="SELECT inv_id,ph_id,qyt FROM sales WHERE id='$value' ";
            $result=$this->connect()->query($sql);
            if ($result->num_rows>0) {
              while($row=$result->fetch_assoc()){
                $arr[$value]=$row;
